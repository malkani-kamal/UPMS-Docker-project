...
function networkUp() {
  echo "Before checkPrereqs"  
  checkPrereqs

  echo "compose file kamal $COMPOSE_FILE end"
  echo "compose file kafka kamal $COMPOSE_FILE_KAFKA end"
  echo "compose file couch kamal $COMPOSE_FILE_COUCH end"
  echo "image tab kamal $IMAGETAG end"
  
  # generate artifacts if they don't exist
  if [ ! -d "crypto-config" ]; then
   echo "crypto-config does not exits"
    generateCerts
    replacePrivateKey
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/upm.tietonetworks.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/dhl.tietonetworks.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
function generateChannelArtifacts() {
  ...
  if [ "$CONSENSUS_TYPE" == "solo" ]; then
      #configtxgen -profile UPMOrgOrdererGenesis -channelID $CHANNEL_NAME -outputBlock ./channel-artifacts/genesis.block
      configtxgen -profile UPMOrgOrdererGenesis -channelID $CHANNEL_NAME -outputBlock ./channel-artifacts/genesis.block
      echo "CONSESUS_TYPE='$CONSENSUS_TYPE'. Built"
  elif [ "$CONSENSUS_TYPE" == "kafka" ]; then
    configtxgen -profile TietoDevModeKafka -channelID byfn-sys-channel -outputBlock ./channel-artifacts/orderer.genesis.block
  else
    set +x
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile UPMOrgChannel -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID $CHANNEL_NAME
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile UPMOrgChannel -outputAnchorPeersUpdate ./channel-artifacts/UpmMSPanchors.tx -channelID $CHANNEL_NAME -asOrg UpmMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile UPMOrgChannel -outputAnchorPeersUpdate ./channel-artifacts/DhlMSPanchors.tx -channelID $CHANNEL_NAME -asOrg DhlMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
